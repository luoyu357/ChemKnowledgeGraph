    ...
    with tempfile.NamedTemporaryFile(delete=False) as tf:
        for record in result['records']:
            for name in record.get('names', []):
                tf.write(('%s\n' % name).encode('utf-8'))
    subprocess.call(['java', '-jar', 'opsin-cli-2.7.0-jar-with-dependencies.jar', '--allowRadicals', '--wildcardRadicals',
        ...
        if validate_record(record):
            ...
            if 'quantum_yields' in record:
                for qy in record['quantum_yields']:
                    if 'value' in qy:
                        if not qtum_ylds_min_range <= float(qy['value'].replace("–", '-').replace(',', '').replace('10-3','0.001').replace('<','')) <= qtum_ylds_max_range:
